...
int CScriptVar::getInt() {
    ...
    if (isInt() || isBool()) return strtol(data.c_str(),0,0);
    if (isNull()) return 0;
...
const string &CScriptVar::getString() {
	...
	if (isBool()) return getInt()==0 ? s_false : s_true;
...
//    CScriptVarLink *a = expression(execute);
    CScriptVarLink *b;
    while ((set==LEX_EQUAL && (l->tk==LEX_EQUAL || l->tk==LEX_NEQUAL ||
           l->tk==LEX_TYPEEQUAL || l->tk==LEX_NTYPEEQUAL))
		   ||
		   (set=='<' && (l->tk==LEX_LEQUAL || l->tk==LEX_GEQUAL ||
							l->tk=='<' || l->tk=='>'))) {
        int op = l->tk;
        l->match(l->tk);
